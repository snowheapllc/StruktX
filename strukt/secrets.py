...
class AWSSecretsManager:
    ...
    def __init__(self, 
        ...
        if not BOTO3_AVAILABLE:
            self._log.warning("boto3 not available, secrets will be fetched from environment variables")
    ...
    def get_secrets(self) -> Dict[str, Any]:
        ...
        if BOTO3_AVAILABLE and self.region_name and self.secret_name:
            try:
                ...
                self._log.error(f"Failed to fetch secrets from AWS Secrets Manager: {e}")
                raise e
        else:
            raise ValueError("AWS Secrets Manager is not available or missing required configuration")
    ...
    def inject_secrets_into_env(self) -> None:
        ...
        try:
            ...
            self._log.info(f"Injected {len(secrets)} secrets into environment variables")
...
def inject_aws_secrets(region_name: str = "", 
                      secret_name: str = "",
                      access_key: Optional[str] = None,
