...
def build_fastapi_app(
    ...
    *,
    ...
    """
    try:
        from fastapi import FastAPI, Header, HTTPException, Body, APIRouter, Request, Response
    ...
    def _normalize_json_schema(schema: Any) -> Dict[str, Any]:
        ...
        if "type" in normalized:
            ...
            if isinstance(type_value, list):
                ...
                if "object" in type_value:
                    ...
                    normalized["type"] = non_null_types[0] if non_null_types else "object"
                else:
                    normalized["type"] = type_value[0] if type_value else "object"
            # Keep valid JSON Schema types as-is
            elif type_value in ["null", "boolean", "object", "array", "number", "string", "integer"]:
        ...
        if "properties" in normalized and isinstance(normalized["properties"], dict):
            for prop_name, prop_schema in normalized["properties"].items():
                ...
                if isinstance(prop_schema, dict) and ("properties" in prop_schema or "items" in prop_schema or "additionalProperties" in prop_schema):
                    normalized["properties"][prop_name] = _normalize_json_schema(prop_schema)
        ...
        if "additionalProperties" in normalized and isinstance(normalized["additionalProperties"], dict):
            normalized["additionalProperties"] = _normalize_json_schema(normalized["additionalProperties"])
        
        return normalized

    def _handle_mcp_request(request_body: Dict[str, Any], request_id: Any) -> Dict[str, Any]:
        ...
        # Handle initialized notification (no response needed)
            ...
            try:
                ...
                for i, tool in enumerate(tools_raw):
                    print(f"DEBUG: Tool {i}: {tool.get('name', 'unnamed')} - outputSchema: {tool.get('outputSchema', 'none')}")
                    ...
                    if "outputSchema" in tool_copy:
                        ...
                        if output_schema:
                            try:
                                ...
                                normalized_schema = _normalize_json_schema(output_schema)
                                print(f"DEBUG: Tool {i} normalized schema: {normalized_schema}")
                                tool_copy["outputSchema"] = normalized_schema
                            except Exception as e:
                                # If normalization fails, remove outputSchema entirely
                                print(f"WARNING: Failed to normalize schema for tool {i} ({tool.get('name', 'unknown')}): {e}")
                ...
                for i, tool in enumerate(tools):
                    print(f"DEBUG: Final tool {i}: {tool.get('name', 'unnamed')} - has outputSchema: {'outputSchema' in tool}")
                ...
                print(f"DEBUG: Complete tools/list response: {json_module.dumps(result, indent=2)}")
    ...
    async def mcp_get_endpoint(
        ...
        if ("text/event-stream" in accept or 
            "streamable" in user_agent.lower()):
            async def streamable_response():
                ...
                try:
                    ...
                    if request_param:
                        ...
                        request_data = json.loads(body.decode('utf-8')) if body else None
                    ...
                    if request_data:
                        ...
                        error_response = {
                            "jsonrpc": "2.0",
                            "id": None,
                            "error": {"code": -32700, "message": "Parse error: Invalid JSON in request"}
                        }
    ...
    class RawMCPEndpoint:
        async def __call__(self, scope, receive, send):
            ...
            # Handle StreamableHTTP connection
            if ("text/event-stream" in accept or
                "streamable-http" in content_type or
                "streamable" in user_agent.lower()):
    ...
    app.router.add_route("/mcp", RawMCPEndpoint(), methods=["GET", "POST", "PUT", "DELETE", "PATCH", "OPTIONS"])
    if prefix != "/mcp":
        app.router.add_route(prefix, RawMCPEndpoint(), methods=["GET", "POST", "PUT", "DELETE", "PATCH", "OPTIONS"])
