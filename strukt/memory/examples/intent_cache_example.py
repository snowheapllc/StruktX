...
from ..memory_config import MemoryConfig, create_default_memory_config
...
class WeatherHandler(SemanticCacheHandlerMixin, CacheAwareHandler):
    ...
    def extract_cache_data(self, state: InvocationState, result: HandlerResult) -> WeatherCacheData:
        ...
        return WeatherCacheData(
            ...
            metadata={"query": state.text, "timestamp": datetime.now(timezone.utc).isoformat()}
        )
    
    def apply_cached_data(self, state: InvocationState, cached_data: WeatherCacheData) -> HandlerResult:
...
def demonstrate_different_strategies():
    ...
    fuzzy_config = IntentCacheConfig(
        enabled=True,
        default_strategy=CacheStrategy.FUZZY,
    ...
    for strategy_name, config in strategies:
        ...
        for i, query in enumerate(test_queries):
            ...
            if match:
                print(f"  Query {i+1}: MATCH (similarity: {match.similarity_score:.2f})")
            else:
